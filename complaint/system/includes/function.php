...
function base_url($sPartUrl = null) {
	if ($sPartUrl) {
		$sWebPath = rtrim(Config::get('path.root.web'), '/').'/'.trim($sPartUrl, '/');
		return $sWebPath;
	}
...
function filter_uri($sUri, $sChars) {
	if ( !empty($sUri) && !empty($sChars) && !preg_match('/^['.$sChars.']+$/i'.(UTF8_ENABLED ? 'u' : ''), $sUri)) {
		return false;
	}
	return true;
}
...
function func_getIp() {
$Ip = $intIp = isset($_SERVER['REMOTE_ADDR']) ? $_SERVER['REMOTE_ADDR'] : '127.0.0.1';
